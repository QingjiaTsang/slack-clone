...
const SignUp = ({ onToggleAuthCard: onToggleAuthForm }: TSignUpProps) => {
  ...
  const onSignUpFormSubmit = async (data: { email: string; name: string; password: string }) => {
    setIsLoading(true)
    try {
  ...
  const onSignInWithOAuth = async (provider: 'github' | 'google') => {
    ...
    try {
      ...
      toast.error(`Sign up failed. Please check your email and password and try again.`)
      setIsLoading(false)
    }
  }
      ...
      <Card className={cn('w-[350px]', { 'opacity-50 pointer-events-none': isLoading })}>
        ...
        <CardContent>
          {!email ? (
            ...
            <OTPVerificationForm onSubmit={onSignInWithOTPFormSubmit} email={email} />
          )}
        ...
        <CardFooter>
          <div className="text-sm text-gray-400 space-x-1">
            ...
            <span className="text-blue-500 cursor-pointer" onClick={onToggleAuthForm}>Sign in</span>
          </div>
        </CardFooter>
      </Card>
    </div>
